...
url_sber_metal = (
    "https://www.sberbank.com/proxy/services/rates/public/actualIngots"
)
url_currencies = "https://ratestats.com/"


async def fetch_data(
    url, params=None, data=None, headers=None, is_post=False, method="text"
):
    if headers is None:
...
async def metals_sber():
    ...
    metals_id = {"Ag": "A99", "Au": "A98", "Pt": "A76", "Pd": "A33"}
...
async def currencies():
    ...
    tickers_raw = bs_currencies.find_all(
        "a", class_="b-rates__ticker-name b-rates__legend_small"
    )
    names = [ticker.text for ticker in tickers_raw]

    prices_block = bs_currencies.find(
        "div", class_="row b-rates__item_highlighted b-rates_small-text"
    )
    prices_raw = prices_block.find_all("div", class_="b-rates__amount")
    prices = [
        float(price.find("span").text.replace(",", ".")).__round__(2)
    ...
    performances = [
        float(
            per.find("div", class_="b-rates__delta").text.replace(",", ".")
        ).__round__(2)
        * -1
        for per in performances_raw
    ]
...
async def markets_main():
    ...
    response = "📊 **Market Overview: (Crypto, Stocks - NOT AVAILABLE), Metals & Currencies**"
